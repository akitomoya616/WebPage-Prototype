...
function begin_play(){
	var a=document.getElementById("player1_id");
	var b=document.getElementById("player2_id");
	var c=document.getElementById("turn_info");
	if(started==true){
...
function reset_play(){
	var a=document.getElementById("player1_id");
	var b=document.getElementById("player2_id");
	var c=document.getElementById("move_text_id");
	var d=document.getElementById("turn_info");
	started=false;
	a.disabled=false;
	b.disabled=false;
	a.value="";
	b.value="";
	c.value="";
	d.innerHTML="Game has not begin.";
...
function play() {
	if(started==true){
		...
		var b=document.getElementById("turn_info");
		var c;
		var d=false;
		var e;
		var f=-1;
		...
		for(var i=0;i<board_state.length;i++){
      		if(table_ids[i]==a.value){
        		...
        		if(board_state[i]==-1){
          			...
          			document.getElementById(a.value).innerHTML=b.innerHTML[13];
          			b.innerHTML=c;
        		}
        		else{
          			alert("Invalid move. Location already been taken. Please re-enter the move again.")
        		}
        		break;
      		}
		}
    	if(d==false){
      		alert("Invalid move. Location not found. Please re-enter the move again.")
      		a.value="";
    	}
    	if(board_state[0]==board_state[1]&&board_state[1]==board_state[2]&&board_state[0]==e){
    		f=e;
    	}
    	else if(board_state[3]==board_state[4]&&board_state[4]==board_state[5]&&board_state[3]==e){
    		f=e;
    	}
    	else if(board_state[6]==board_state[7]&&board_state[7]==board_state[8]&&board_state[6]==e){
    		f=e;
    	}
    	else if(board_state[0]==board_state[3]&&board_state[3]==board_state[6]&&board_state[0]==e){
    		f=e;
    	}
    	else if(board_state[1]==board_state[4]&&board_state[4]==board_state[7]&&board_state[1]==e){
    		f=e;
    	}
    	else if(board_state[2]==board_state[5]&&board_state[5]==board_state[8]&&board_state[2]==e){
    		f=e;
    	}
    	else if(board_state[0]==board_state[4]&&board_state[4]==board_state[8]&&board_state[0]==e){
    		f=e;
    	}
    	else if(board_state[2]==board_state[4]&&board_state[4]==board_state[6]&&board_state[2]==e){
    		f=e;
    	}
    	if(f!=-1){
    		if(f==1){
